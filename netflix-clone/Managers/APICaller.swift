...
struct Constants {
    static let API_KEY = "8273e3e7f985fb3f181c9d4487907a08"
...
class APICaller {
    ...
    func search(with query: String, completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.BASE_URL)/3/search/movie?api_key=\(Constants.API_KEY)&query=J\(query)") else { return}
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else {
                return
            }
